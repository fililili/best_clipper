...
struct half_chain_t {
  ...
  point source_node(const chain_group &chains) const {
    if (is_forward())
      return chains.points[chains.offsets[chain_id()]];
    else
      return chains.points[chains.offsets[chain_id() + 1] - 1];
  }
  point target_node(const chain_group &chains) const {
    if (is_forward())
      return chains.points[chains.offsets[chain_id() + 1] - 1];
    else
      return chains.points[chains.offsets[chain_id()]];
  }
  point next_along_source(const chain_group &chains) const {
    if (is_forward())
