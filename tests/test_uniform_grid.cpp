...
static bool boxes_overlap(const box& a, const box& b) {
    return !(bg::get<1, 0>(a) < bg::get<0, 0>(b) || bg::get<0, 0>(a) > bg::get<1, 0>(b)
          || bg::get<1, 1>(a) < bg::get<0, 1>(b) || bg::get<0, 1>(a) > bg::get<1, 1>(b));
}
...
TEST(UniformGrid, TwoDisjoint) {
    ...
    int count = 0;
    g.query_intersects(q1, [&](size_t i) { if (boxes_overlap(q1, boxes[i])) count++; });
    EXPECT_EQ(count, 1);

    auto q2 = box{point{50, 50}, point{60, 60}};
    count = 0;
    g.query_intersects(q2, [&](size_t i) { if (boxes_overlap(q2, boxes[i])) count++; });
    EXPECT_EQ(count, 0);
}
...
TEST(UniformGrid, PointQueryOutside) {
    ...
    int count = 0;
    g.query_intersects(q, [&](size_t i) { if (boxes_overlap(q, boxes[i])) count++; });
    EXPECT_EQ(count, 0);
}
