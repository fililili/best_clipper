...
ring make_rect(int32_t x1, int32_t y1, int32_t x2, int32_t y2) {
  ring r;
...
ring make_hole(int32_t x1, int32_t y1, int32_t x2, int32_t y2) {
  ring r;
  r.push_back(point{x1, y1});
  r.push_back(point{x2, y1});
  r.push_back(point{x2, y2});
...
polygon make_rect_poly(int32_t x1, int32_t y1, int32_t x2, int32_t y2) {
  polygon p;
...
TEST(ManhattanUnion, TwoSeparateRects) {
  ...
  multi_polygon b;
  b.push_back(make_rect_poly(5, 5, 7, 7));
...
TEST(ManhattanUnion, OverlappingRects) {
  ...
  multi_polygon b;
  b.push_back(make_rect_poly(2, 2, 5, 5));
  ASSERT_TRUE(bg::is_valid(a)) << "a: " << bg::wkt(a);
  ASSERT_TRUE(bg::is_valid(b)) << "b: " << bg::wkt(b);
...
TEST(ManhattanUnion, AdjacentRects) {
  ...
  a.push_back(make_rect_poly(0, 0, 2, 2));
  multi_polygon b;
  b.push_back(make_rect_poly(2, 0, 4, 2));
  ASSERT_TRUE(bg::is_valid(a)) << "a: " << bg::wkt(a);
  ASSERT_TRUE(bg::is_valid(b)) << "b: " << bg::wkt(b);
...
TEST(ManhattanUnion, CornerTouching) {
  ...
  a.push_back(make_rect_poly(0, 0, 2, 2));
  multi_polygon b;
  b.push_back(make_rect_poly(2, 2, 4, 4));
  ASSERT_TRUE(bg::is_valid(a)) << "a: " << bg::wkt(a);
  ASSERT_TRUE(bg::is_valid(b)) << "b: " << bg::wkt(b);
...
TEST(ManhattanUnion, RectWithHole) {
  ...
  polygon p1;
  p1.outer() = make_rect(0, 0, 6, 6);
  p1.inners().push_back(make_hole(2, 2, 4, 4));
  multi_polygon a;
  a.push_back(p1);
  multi_polygon b;
  b.push_back(make_rect_poly(3, 3, 8, 8));
  ASSERT_TRUE(bg::is_valid(a)) << "a: " << bg::wkt(a);
  ASSERT_TRUE(bg::is_valid(b)) << "b: " << bg::wkt(b);
...
TEST(ManhattanUnion, RectPartiallyFillsHole) {
  ...
  polygon p1;
  p1.outer() = make_rect(0, 0, 8, 8);
  p1.inners().push_back(make_hole(2, 2, 6, 6));
  multi_polygon a;
  a.push_back(p1);
  multi_polygon b;
  b.push_back(make_rect_poly(4, 4, 7, 7));
  ASSERT_TRUE(bg::is_valid(a)) << "a: " << bg::wkt(a);
  ASSERT_TRUE(bg::is_valid(b)) << "b: " << bg::wkt(b);
...
TEST(ManhattanIntersection, RectWithHole) {
  ...
  a.push_back(p1);
  multi_polygon b;
  b.push_back(make_rect_poly(1, 1, 5, 5));
  ASSERT_TRUE(bg::is_valid(a)) << "a: " << bg::wkt(a);
  ASSERT_TRUE(bg::is_valid(b)) << "b: " << bg::wkt(b);
...
TEST(ManhattanSelfOr, LShapedSelfOr) {
  ...
  EXPECT_TRUE(bg::is_valid(result)) << "result: " << bg::wkt(result);
  EXPECT_EQ(result.size(), 1u);
  EXPECT_DOUBLE_EQ(bg::area(result),
                   16.0); // 10+6 (line overlap only, no area loss)
}
...
TEST(ManhattanEdgeCase, VeryNarrowOverlap) {
  ...
  a.push_back(make_rect_poly(0, 0, 10, 10));
  multi_polygon b;
  b.push_back(make_rect_poly(9, 0, 11, 10));
  ASSERT_TRUE(bg::is_valid(a)) << "a: " << bg::wkt(a);
  ASSERT_TRUE(bg::is_valid(b)) << "b: " << bg::wkt(b);
...
TEST(ManhattanEdgeCase, LastColumnOverlap) {
  ...
  multi_polygon a;
  a.push_back(make_rect_poly(0, 0, 5, 5));
  multi_polygon b;
  b.push_back(make_rect_poly(4, 2, 8, 3));
  ASSERT_TRUE(bg::is_valid(a)) << "a: " << bg::wkt(a);
  ASSERT_TRUE(bg::is_valid(b)) << "b: " << bg::wkt(b);
...
TEST(ManhattanLarge, RectWith100Holes) {
  ...
  {
    polygon p;
    p.outer() = make_rect(0, 0, 100, 100);
    for (int i = 0; i < 10; i++)
      for (int j = 0; j < 10; j++)
        p.inners().push_back(
            make_hole(2 + i * 10, 2 + j * 10, 8 + i * 10, 8 + j * 10));
    a.push_back(p);
  }
  multi_polygon b;
...
TEST(ManhattanLarge, RectWith20Holes) {
  ...
  multi_polygon a;
  {
    ...
    for (int i = 0; i < 4; i++)
      for (int j = 0; j < 5; j++)
        p.inners().push_back(
            make_hole(2 + i * 10, 2 + j * 20, 8 + i * 10, 8 + j * 20));
    a.push_back(p);
  }
  multi_polygon b;
  b.push_back(make_rect_poly(1, 1, 52, 99));
...
TEST(ManhattanLarge, RectWithCoincidentHole) {
  ...
  multi_polygon a;
  {
    ...
    p.outer() = make_rect(0, 0, 100, 100);
    p.inners().push_back(
        make_hole(52, 2, 58, 8));                // hole column 5, left at x=52
    p.inners().push_back(make_hole(2, 2, 8, 8)); // hole column 0, inside B
    a.push_back(p);
  }
  multi_polygon b;
  b.push_back(make_rect_poly(
      1, 1, 52, 99)); // B's right edge at x=52 coincides with hole left
  ASSERT_TRUE(bg::is_valid(a)) << "a: " << bg::wkt(a);
  ASSERT_TRUE(bg::is_valid(b)) << "b: " << bg::wkt(b);
...
TEST(ManhattanLarge, RectWithTwoCoincidentHoles) {
  ...
  multi_polygon a;
  {
    ...
    p.inners().push_back(
        make_hole(52, 12, 58, 18));              // hole 1, x=52..58, y=12..18
...
TEST(ManhattanXor, TwoSeparateRects) {
  ...
  auto result = symmetric_difference(a, b);
  multi_polygon un, inter, expected;
  bg::union_(a, b, un);
...
TEST(ManhattanDifference, BcontainsA) {
  ...
  a.push_back(make_rect_poly(2, 2, 5, 5));
  multi_polygon b;
  b.push_back(make_rect_poly(0, 0, 10, 10));
  ASSERT_TRUE(bg::is_valid(a)) << "a: " << bg::wkt(a);
  ASSERT_TRUE(bg::is_valid(b)) << "b: " << bg::wkt(b);
