...
struct RandPolygons {
    ...
    explicit RandPolygons(int n) {
        ...
        for (int i = 0; i < n; i++)
            polys.push_back(gen_poly(center(rng), center(rng), radius_dist(rng), rng));
    }
};
...
BENCHMARK_DEFINE_F(CmpUnionFixture, BestClipper_Union)(benchmark::State& state) {
    for (auto _ : state)
...
BENCHMARK_DEFINE_F(CmpIntersectionFixture, BestClipper_Intersection)(benchmark::State& state) {
    for (auto _ : state)
        benchmark::DoNotOptimize(intersection(a, b));
}
BENCHMARK_REGISTER_F(CmpIntersectionFixture, BestClipper_Intersection) CMP_INTER_ARGS;
BENCHMARK_DEFINE_F(CmpIntersectionFixture, Clipper2_Intersection)(benchmark::State& state) {
    for (auto _ : state)
        benchmark::DoNotOptimize(Clipper2Lib::Intersect(paths_a, paths_b, Clipper2Lib::FillRule::NonZero));
}
BENCHMARK_REGISTER_F(CmpIntersectionFixture, Clipper2_Intersection) CMP_INTER_ARGS;
...
BENCHMARK_DEFINE_F(CmpXorFixture, Clipper2_Xor)(benchmark::State& state) {
    for (auto _ : state)
        benchmark::DoNotOptimize(Clipper2Lib::Xor(paths_a, paths_b, Clipper2Lib::FillRule::NonZero));
}
...
BENCHMARK_DEFINE_F(CmpDifferenceFixture, BestClipper_Difference)(benchmark::State& state) {
    for (auto _ : state)
        benchmark::DoNotOptimize(difference(a, b));
}
BENCHMARK_REGISTER_F(CmpDifferenceFixture, BestClipper_Difference) CMP_DIFF_ARGS;
BENCHMARK_DEFINE_F(CmpDifferenceFixture, Clipper2_Difference)(benchmark::State& state) {
    for (auto _ : state)
        benchmark::DoNotOptimize(Clipper2Lib::Difference(paths_a, paths_b, Clipper2Lib::FillRule::NonZero));
}
...
BENCHMARK_DEFINE_F(CmpSelfOrFixture, BestClipper_SelfOr)(benchmark::State& state) {
    for (auto _ : state)
