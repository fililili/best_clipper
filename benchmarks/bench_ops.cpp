...
struct RectangleUnionFixture : benchmark::Fixture {
    void SetUp(const benchmark::State& state) override {
        ...
        for (int i = 0; i < n; i++) {
            a.push_back(polygon{{{{0+2*i,0+2*i},{0+2*i,2+2*i},{2+2*i,2+2*i},{2+2*i,0+2*i},{0+2*i,0+2*i}}}});
            b.push_back(polygon{{{{1+2*i,1+2*i},{1+2*i,3+2*i},{3+2*i,3+2*i},{3+2*i,1+2*i},{1+2*i,1+2*i}}}});
        }
    }
    multi_polygon a, b;
};
...
struct RectangleSelfOrFixture : benchmark::Fixture {
    void SetUp(const benchmark::State& state) override {
        ...
        poly.reserve(n);
        for (int i = 0; i < n; i++)
            poly.push_back(polygon{{{{0+i,0+i},{0+i,2+i},{2+i,2+i},{2+i,0+i},{0+i,0+i}}}});
    }
    multi_polygon poly;
};
